...
interface TokenGridProps {
  ...
  title: string;
  isLoading?: boolean;
  shouldCreateHasMoreData?: boolean;
}
...
const TokenGrid = ({
  ...
  useEffect(() => {
    if (Array.isArray(tokens)) {
      ...
      if (shouldCreateHasMoreData) {
        setDisplayedTokens(validTokens.slice(0, 20));
        setRemainingTokens(validTokens.slice(20));
      } else {
        setDisplayedTokens(validTokens.slice(0,20));
        setRemainingTokens([]);
      }
  ...
  }, [tokens, shouldCreateHasMoreData]);

  const fetchMore = () => {
    if (!shouldCreateHasMoreData || loadingMore || remainingTokens.length === 0) return;

    setLoadingMore(true);
    setTimeout(() => {
      const more = remainingTokens.slice(0, 20);
      const leftover = remainingTokens.slice(20);

      setDisplayedTokens((prev) => [...prev, ...more]);
      setRemainingTokens(leftover);
      setLoadingMore(false);
    }, 500); // Simulate load delay
  };

  useEffect(() => {
    if (!shouldCreateHasMoreData || remainingTokens.length === 0) return;
  ...
  return (
    <div className="w-full">
      ...
      >
        ...
        {shouldCreateHasMoreData && remainingTokens.length > 0 && !loadingMore && (
          <div
            ref={loaderRef}
